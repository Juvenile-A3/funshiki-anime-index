...
for i,seed in enumerate(json.loads((root/'config/subjects-seed.json').read_text(encoding='utf-8'))):
    path=cache/f'{i}.json'
    if not path.exists():
        req=urllib.request.Request('https://api.bgm.tv/v0/search/subjects?limit=5',data=json.dumps({'keyword':seed['query'],'filter':{'type':[2]},'sort':'match'}).encode(),headers={'Content-Type':'application/json','User-Agent':'FunshikiAnimeIndex/0.1 (public metadata prototype)'})
